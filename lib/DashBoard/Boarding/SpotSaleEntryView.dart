...
import '../Admin2/Admin2Dashboard.dart';
import 'BoardingDashboard.dart';
class SpotSaleEntryView extends StatefulWidget {
  final DateTime? fromDate;
  final DateTime? toDate;

  const SpotSaleEntryView({super.key, this.fromDate, this.toDate});

  @override
  State<SpotSaleEntryView> createState() => _SpotSaleEntryViewState();
}

class _SpotSaleEntryViewState extends State<SpotSaleEntryView> {
  DateTime? fromDate;
  DateTime? toDate;
  ...
  Future<void> fetchData() async {
    ...
    await objfun.apiAllinoneSelectArray(
      "${objfun.apiGetSpotSaleEntry}${objfun.Comid}&Fromdate=$from&Todate=$to&Id=$Id",
      null,
      header,
      context,
    ).then((data) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(" Spot SaleOrder Entries"),
        centerTitle: true,
        elevation: 3,
      ),
                        ...
                        child: GestureDetector(
                          ...
                          if (objfun.storagenew.getString('RulesType') == "ADMIN") {
                            ...
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => Admin2Dashboard(
                                    editId: Id,initialTabIndex: 9
                                ),
                              ),
                            );
                          }
                            ...
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => BoardingDashboard(
                                    editId: Id,viewId: 2
                                ),
                              ),
                            );
                            ...
                            print("Container tapped" + Id.toString());
                          },
                          child: Column(
