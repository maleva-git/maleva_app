...
class SaleOrderViewState extends State<SaleOrderViews> {
  ...
  Future _shareDOConvert( int Id , int BillNo) async {
    bool result =
    await objfun.ConfirmationMsgYesNo(context, "Do you want to prepare DO?");
    if (result == false) {
      return;
    }
    Map<String?, dynamic> master = {};
    ...
    await objfun
        .apiAllinoneSelectArray(
        "${objfun.apiViewDOConvert}$BillNo",
        master,
        header,
        context)
        .then((resultData) async {
      if (resultData != "") {
        ...
        if (value.IsSuccess == true) {
         // objfun.startDownloading(value.data1);
  ...
  Future _shareInvoice( int Id , int BillNo) async {

    bool result =
    await objfun.ConfirmationMsgYesNo(context, "Do you want to prepare Invoice?");
    if (result == false) {
    return;
    }
    Map<String?, dynamic> master = {};
