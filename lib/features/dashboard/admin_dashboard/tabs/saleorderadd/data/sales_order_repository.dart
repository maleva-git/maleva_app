...
class SalesOrderRepository {
  ...
  Future<Map<String, dynamic>> fetchInitialData(String billType) async {
    ...
    final maxOrderResponse = await ApiClient.postRequest("${objfun.apiMaxSaleOrderNo}$comid&BillType=$billType", null);
    final agentCompanyResponse = await ApiClient.postRequest("${objfun.apiSelectAgentCompany}$comid", null);
    final employeeResponse = await ApiClient.postRequest("${objfun.apiSelectEmployee}$comid&AccountName=&Type=Operation", null);
  ...
  Future<Map<String, dynamic>> fetchMasterData(int jobMasterRefId, int agentCompanyRefId, int customerRefId) async {
    final customerResponse = await ApiClient.postRequest("${objfun.apiSelectCustomer}$comid", null);
    ...
    final currencyResponse = await ApiClient.postRequest("${objfun.apiGetCurrencyValue}$comid&CustomerRefId=$customerRefId", null);
  ...
  Future<Map<String, dynamic>> fetchJobTypeDependencies(int jobTypeId) async {
    final jobStatusResponse = await ApiClient.postRequest("${objfun.apiSelectAllJobStatus}$comid&JobMasterRefId=$jobTypeId", null);
    final comboS1Response = await ApiClient.postRequest("${objfun.apiGetComboS1}$comid&JobMasterRefId=$jobTypeId", null);
  ...
  Future<ResponseViewModel?> saveSalesOrder(List<Map<String, dynamic>> master) async {
    final response = await ApiClient.postRequest('${objfun.apiInsertSalesOrder}?Comid=$comid', master);
    return response != null ? ResponseViewModel.fromJson(response) : null;
  }

  Future<void> confirmEnquiry(int enquiryId) async {
    await ApiClient.postRequest('${objfun.apiUpdateEnquiryMaster}$enquiryId&Comid=$comid&StatusName=CONFIRMED', null);
  }
}