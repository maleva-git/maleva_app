...
import 'package:maleva/features/dashboard/admin_dashboard/tabs/saleorderview/bloc/saleorderview_event.dart';
import 'package:maleva/features/dashboard/admin_dashboard/tabs/saleorderview/bloc/saleorderview_state.dart';
...
class SaleOrderBloc extends Bloc<SaleOrderEvent, SaleOrderState> {
  ...
  SaleOrderBloc({required this.repository}) : super(SaleOrderState(
    checkBoxValueLEmp:
    objfun.storagenew.getString('RulesType') != 'ADMIN',
  )) {
  ...
  Future<void> _fetchData(Emitter<SaleOrderState> emit) async {
    try {
      final int empRefId =
      state.checkBoxValueLEmp ? objfun.EmpRefId : state.empId;
      ...
      if (resultData != null &&
        ...
        objfun.SaleOrderMasterList = masters;
        objfun.SaleOrderDetailList = details;
