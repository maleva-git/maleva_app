...
class _SpotSaleEntryBody extends StatelessWidget {
  ...
  Widget _buildFormContent(
    ...
    final jobTypeName = AppGlobals.JobTypeList
        .where((j) => j.Id.toString() == s.selectedJobType)
        .map((j) => j.Name ?? '')
        .firstOrNull ?? '';

    final jobStatusName = AppGlobals.JobStatusList
...
class _JobTypeSelectPageState extends State<_JobTypeSelectPage> {
  ...
  void initState() {
    super.initState();
    _filtered = List.from(AppGlobals.JobTypeList);
  }

  void _search(String q) {
    setState(() {
      _filtered = q.isEmpty
          ? List.from(AppGlobals.JobTypeList)
...
class _JobStatusSelectPageState
  ...
  void initState() {
    super.initState();
    _filtered = List.from(AppGlobals.JobStatusList);
  }

  void _search(String q) {
    setState(() {
      _filtered = q.isEmpty
          ? List.from(AppGlobals.JobStatusList)
...
class _PortSelectPageState extends State<_PortSelectPage> {
  ...
  void initState() {
    super.initState();
    _filtered = List.from(AppGlobals.Portlist);
  }

  void _search(String q) {
    setState(() {
      _filtered = q.isEmpty
          ? List.from(AppGlobals.Portlist)
          : AppGlobals.Portlist
          .where((p) => (p.name ?? '')
          .toUpperCase()
          .contains(q.toUpperCase()))
