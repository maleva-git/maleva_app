...
class _SalesOrderAddBody extends StatefulWidget {
  ...
  @override
  State<_SalesOrderAddBody> createState() => _SalesOrderAddBodyState();
}
...
class _SalesOrderAddBodyState extends State<_SalesOrderAddBody> with TickerProviderStateMixin {
    ...
    if (!_controllers.containsKey(key)) {
      ...
      if (currentText != stateValue) {
        _controllers[key]!.value = _controllers[key]!.value.copyWith(
          text: stateValue,
          selection: TextSelection.collapsed(offset: stateValue.length),
        );
      }
    }
      ...
      listener: (context, state) {
        if (state is SalesOrderAddLoaded) {
          if (state.savedMessage != null) {
            toastMsg(state.savedMessage!, '', context);
          }
          if (state.isSaved) {
            Navigator.pushReplacement(
                context, MaterialPageRoute(builder: (_) => const SalesOrdersAdd()));
          }
        }
      },
      builder: (context, state) {
        if (state is SalesOrderAddLoading || state is SalesOrderAddInitial) {
          return const Scaffold(
            backgroundColor: colour.surface,
            body: Center(child: SpinKitFoldingCube(color: colour.brand, size: 35)),
          );
        }
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context, SalesOrderAddLoaded state) {
    return AppBar(
      elevation: 0,
      ...
      actions: [
        _appBarBtn(
          ...
          onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SaleOrderView())),
        ),
        _appBarBtn(
          label: 'SAVE',
          enabled: state.fieldPermission["SAVE"] == true,
          onPressed: () async {
            final confirm = await ConfirmationMsgYesNo(context, "Do You Want to Save ?");
  ...
  Widget _appBarBtn({required String label, required bool enabled, required VoidCallback onPressed}) {
    return Padding(
  ...
  Widget _buildTabBody(BuildContext context, SalesOrderAddLoaded state, double width, double height) {
  ...
  Widget _buildBottomNav(BuildContext context, SalesOrderAddLoaded state) {
    final List<IconData> navIcons = [
      Icons.info_outline_rounded, Icons.comment_outlined, Icons.directions_boat_filled,
      Icons.directions_boat_filled_outlined, Icons.local_shipping_rounded, Icons.format_list_bulleted_rounded,
    ];
    ...
    return Container(
      ...
      child: SalomonBottomBar(
        ...
        items: navIcons.map((icon) => SalomonBottomBarItem(icon: Icon(icon, size: 22), title: const SizedBox.shrink(), selectedColor: colour.brand)).toList(),
      ),
    );
  }
  ...
  // TAB 1
  // ════════════════════════════════════════════════════
  Widget _buildTab1(BuildContext context, SalesOrderAddLoaded state, double width, double height) {
    ...
    return _tabScroll(children: [
      _sectionCard(children: [
        Row(children: [
          ...
          Expanded(
            flex: 4,
            child: _dateTapBox(
              date: width <= 370 ? DateFormat("dd-MM-yy").format(DateTime.parse(state.dtpSaleOrderdate)) : DateFormat("dd-MM-yyyy").format(DateTime.parse(state.dtpSaleOrderdate)),
              onTap: null,
            ),
          ),
        ]),
      ...
      _sectionCard(children: [
        _searchField(
          ...
          onSearch: () async {
            final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => const Customer(Searchby: 1, SearchId: 0))); if (r != null) { AppGlobals.SelectCustomerList = r; }
            if (r != null) {
              bloc.add(CustomerSelected(AppGlobals.SelectCustomerList.AccountName, AppGlobals.SelectCustomerList.Id));
        ...
        _searchField(
          ...
          onSearch: () async {
            final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => const JobType(Searchby: 1, SearchId: 0))); if (r != null) { AppGlobals.SelectJobTypeList = r; }
            if (r != null) {
              bloc.add(JobTypeSelected(AppGlobals.SelectJobTypeList.Name, AppGlobals.SelectJobTypeList.Id));
        ...
        _searchField(
          ...
          onSearch: () async {
            final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => JobAllStatus(Searchby: 1, SearchId: 0, JobTypeId: state.jobTypeId))); if (r != null) { AppGlobals.SelectAllStatusList = r; }
            if (r != null) {
              bloc.add(JobStatusSelected(AppGlobals.SelectAllStatusList.StatusName, AppGlobals.SelectAllStatusList.Status));
        ...
        _gap(),
        _editField(
          uniqueId: 'txt_remarks', hint: 'Remarks', value: state.txtRemarks, enabled: fp["txtRemarks"] == true,
          onChanged: (v) => bloc.add(UpdateTextField('txtRemarks', v)), minLines: 2, maxLines: 4,
        ),
        _gap(),
        _editField(
          uniqueId: 'txt_dodescription', hint: 'DO Description', value: state.txtDoDescription, enabled: fp["txtDoDescription"] == true,
          onChanged: (v) => bloc.add(UpdateTextField('txtDoDescription', v)), minLines: 3, maxLines: 5, keyboardType: TextInputType.multiline,
        ),
      ]),
      const SizedBox(height: 10),
      Row(
...
if (__navResult1 != null) { AppGlobals.SelectedCommodityName = __navResult1; }
            ...
            if (AppGlobals.SelectedCommodityName.isNotEmpty) {
              ...
              AppGlobals.SelectedCommodityName = "";
            }
          },
          onClear: () => bloc.add(CommoditySelected('')),
        ),
        _gap(),
        _editField(uniqueId: 'txt_weight', hint: 'Weight', value: state.txtWeight, enabled: fp["txtWeight"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtWeight', v))),
        _gap(),
        _editField(uniqueId: 'txt_quantity', hint: 'Quantity', value: state.txtQuantity, enabled: fp["txtQuantity"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtQuantity', v))),
        _gap(),
        if (!state.visibleGC)
          _editField(uniqueId: 'txt_trucksize', hint: 'Truck Size', value: state.txtTruckSize, enabled: fp["txtTruckSize"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtTruckSize', v)))
        else
          Row(children: [
            Expanded(flex: 2, child: _labelText('Truck Size')),
            const SizedBox(width: 10),
            Expanded(flex: 5, child: _styledDropdown<String>(value: state.dropdownValueTruckSize, items: SalesOrderAddBloc.truckSizeList, enabled: true, onChanged: (v) => bloc.add(UpdateDropdown('dropdownValueTruckSize', v)))),
          ]),
        if (state.visibleAWBNo) ...[
          _gap(), _editField(uniqueId: 'txt_awbno', hint: 'AWB NO', value: state.txtAWBNo, enabled: fp["txtAWBNo"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtAWBNo', v))),
        ],
        if (state.visibleBLCopy) ...[
          _gap(), _editField(uniqueId: 'txt_blcopy', hint: 'BL Copy', value: state.txtBLCopy, enabled: fp["txtBLCopy"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtBLCopy', v))),
        ],
        _gap(),
        _searchField(
...
if (__navResult2 != null) { AppGlobals.SelectedCargoName = __navResult2; }
            ...
            if (AppGlobals.SelectedCargoName.isNotEmpty) {
              ...
              AppGlobals.SelectedCargoName = "";
            }
          },
          onClear: () => bloc.add(CargoSelected('')),
        ),
        _gap(),
        _editField(uniqueId: 'txt_ptwno', hint: 'PTW No', value: state.txtPTWNo, enabled: fp["txtPTWNo"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtPTWNo', v))),
      ]),
    ]);
  }
      ...
      _sectionCard(children: [
        ...
        if (state.visibleLShippingAgent) ...[
          _searchField(
            ...
            onSearch: () async {
              final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => const AgentCompany(Searchby: 1, SearchId: 0))); if (r != null) { AppGlobals.SelectAgentCompanyList = r; }
              if (r != null) {
                bloc.add(LAgentCompanySelected(AppGlobals.SelectAgentCompanyList.Name, AppGlobals.SelectAgentCompanyList.Id));
                AppGlobals.SelectAgentCompanyList = AgentCompanyModel.Empty();
              }
            },
        ...
        if (state.visibleLAgentName) ...[
          _searchField(
            ...
            onSearch: () async {
              final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => Agent(Searchby: 1, SearchId: 0, AgentCompanyId: state.lAgentCompanyId))); if (r != null) { AppGlobals.SelectAgentAllList = r; }
              if (r != null) {
                bloc.add(LAgentSelected(AppGlobals.SelectAgentAllList.AgentName, AppGlobals.SelectAgentAllList.Id));
                AppGlobals.SelectAgentAllList = AgentModel.Empty();
              }
            },
            onClear: () => bloc.add(LAgentSelected('', 0)),
          ), _gap(),
        ],
        if (state.visibleLScn) ...[ _editField(uniqueId: 'txt_l_scn', hint: 'SCN', value: state.txtLSCN, enabled: fp["txtLSCN"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtLSCN', v))), _gap() ],
        if (state.visibleLoadingVessel) ...[ _editField(uniqueId: 'txt_loadingvessel', hint: 'Loading Vessel Name', value: state.txtLoadingVessel, enabled: fp["txtLoadingVessel"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtLoadingVessel', v))), _gap() ],
...
if (__navResult4 != null) { AppGlobals.SelectedVesselTypeName = __navResult4; }
              ...
              if (AppGlobals.SelectedVesselTypeName.isNotEmpty) {
                ...
                AppGlobals.SelectedVesselTypeName = "";
              }
            },
  ...
  Widget _buildTab4(BuildContext context, SalesOrderAddLoaded state, double height) {
    ...
    return _tabScroll(children: [
      _sectionCard(children: [
        if (state.visibleOETA) ...[ _dateCheckRow(context: context, label: "ETA", dateStr: state.dtpOETAdate, checkValue: state.checkBoxValueOETA, checkKey: 'checkBoxValueOETA', dateKey: 'dtpOETAdate', enabled: fp["chkOETA"] == true, bloc: bloc, showTime: true), _gap() ],
        if (state.visibleOETB) ...[ _dateCheckRow(context: context, label: "ETB", dateStr: state.dtpOETBdate, checkValue: state.checkBoxValueOETB, checkKey: 'checkBoxValueOETB', dateKey: 'dtpOETBdate', enabled: fp["chkLETB"] == true, bloc: bloc, showTime: true), _gap() ],
        if (state.visibleOETD) ...[ _dateCheckRow(context: context, label: "ETD", dateStr: state.dtpOETDdate, checkValue: state.checkBoxValueOETD, checkKey: 'checkBoxValueOETD', dateKey: 'dtpOETDdate', enabled: fp["chkLETD"] == true, bloc: bloc, showTime: true), _gap() ],
        if (state.visibleOShippingAgent) ...[
          _searchField(
            ...
            onSearch: () async {
              final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => const AgentCompany(Searchby: 1, SearchId: 0))); if (r != null) { AppGlobals.SelectAgentCompanyList = r; }
              if (r != null) {
                bloc.add(OAgentCompanySelected(AppGlobals.SelectAgentCompanyList.Name, AppGlobals.SelectAgentCompanyList.Id));
                AppGlobals.SelectAgentCompanyList = AgentCompanyModel.Empty();
              }
            },
        ...
        if (state.visibleOAgentName) ...[
          _searchField(
            ...
            onSearch: () async {
              final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => Agent(Searchby: 1, SearchId: 0, AgentCompanyId: state.oAgentCompanyId))); if (r != null) { AppGlobals.SelectAgentAllList = r; }
              if (r != null) {
                bloc.add(OAgentSelected(AppGlobals.SelectAgentAllList.AgentName, AppGlobals.SelectAgentAllList.Id));
                AppGlobals.SelectAgentAllList = AgentModel.Empty();
              }
            },
        ...
        if (state.visibleOScn) ...[ _editField(uniqueId: 'txt_o_scn', hint: 'SCN', value: state.txtOSCN, enabled: fp["txtOSCN"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtOSCN', v))), _gap() ],
        if (state.visibleOffVessel) ...[ _editField(uniqueId: 'txt_offvessel', hint: 'Off Vessel Name', value: state.txtOffVessel, enabled: fp["txtOffVessel"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtOffVessel', v))), _gap() ],
...
if (__navResult6 != null) { AppGlobals.SelectedVesselTypeName = __navResult6; }
  ...
  Widget _buildTab5(BuildContext context, SalesOrderAddLoaded state, double height) {
    ...
    return _tabScroll(children: [
      _sectionCard(children: [
        _dateCheckRow(context: context, label: "PickUp Date", dateStr: state.dtpPickUpdate, checkValue: state.checkBoxValuePickUp, checkKey: 'checkBoxValuePickUp', dateKey: 'dtpPickUpdate', enabled: fp["chkPickup"] == true, bloc: bloc, showTime: true), _gap(),
        _dateCheckRow(context: context, label: "Delivery Date", dateStr: state.dtpDeliverydate, checkValue: state.checkBoxValueDelivery, checkKey: 'checkBoxValueDelivery', dateKey: 'dtpDeliverydate', enabled: fp["chkDelivery"] == true, bloc: bloc, showTime: true), _gap(),
        _dateCheckRow(context: context, label: "WH Entry", dateStr: state.dtpWHEntrydate, checkValue: state.checkBoxValueWHEntry, checkKey: 'checkBoxValueWHEntry', dateKey: 'dtpWHEntrydate', enabled: fp["chkWareHouseEntry"] == true, bloc: bloc, showTime: true), _gap(),
        _dateCheckRow(context: context, label: "WH Exit", dateStr: state.dtpWHExitdate, checkValue: state.checkBoxValueWHExit, checkKey: 'checkBoxValueWHExit', dateKey: 'dtpWHExitdate', enabled: fp["chkWareHouseExit"] == true, bloc: bloc, showTime: true),
      ]),
      const SizedBox(height: 10),
      if (state.visibleOrigin || state.visibleDestination)
        _sectionCard(children: [
          if (state.visibleOrigin && !state.visibleGC) ...[ _editField(uniqueId: 'txt_origin_normal', hint: 'Origin', value: state.txtOrigin, enabled: fp["txtOrigin"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtOrigin', v))), _gap() ],
          if (state.visibleDestination && !state.visibleGC) ...[ _editField(uniqueId: 'txt_dest_normal', hint: 'Destination', value: state.txtDestination, enabled: fp["txtDestination"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtDestination', v))), _gap() ],
          if (state.visibleGC) ...[
            _searchField(
              hint: "Origin", uniqueId: 'search_origin_gc', value: state.txtOrigin, enabled: fp["txtOrigin"] == true,
              onSearch: () async {
                final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => const Location(Searchby: 1, SearchId: 0))); if (r != null) { AppGlobals.SelectLocationList = r; }
                if (r != null) {
                  bloc.add(OriginSelected(AppGlobals.SelectLocationList.Location, AppGlobals.SelectLocationList.Id));
            ...
            _searchField(
              ...
              onSearch: () async {
                final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => const Location(Searchby: 1, SearchId: 0))); if (r != null) { AppGlobals.SelectLocationList = r; }
                if (r != null) {
                  bloc.add(DestinationSelected(AppGlobals.SelectLocationList.Location, AppGlobals.SelectLocationList.Id));
                  AppGlobals.SelectLocationList = LocationModel.Empty();
                }
              },
      ...
      const SizedBox(height: 10),
      _sectionCard(children: [
        _sectionLabel('Warehouse Address'), _gap(),
        _addressMultiField(
  ...
  Widget _buildTab6(BuildContext context, SalesOrderAddLoaded state, double width, double height) {
    ...
    return _tabScroll(children: [
      if (state.visibleFORWARDING) ...[
        _fwCard(context, bloc, state, fp, fwNum: 1, visible: state.visibleFW1, dropValue: state.dropdownValueFW1, dropKey: 'dropdownValueFW1', dateStr: state.dtpFW1date, checkValue: state.checkBoxValueFW1, checkKey: 'checkBoxValueFW1', dateKey: 'dtpFW1date', smkValue: state.txtSmk1, smkKey: 'txtSmk1', enRef: state.txtENRef1, enKey: 'txtENRef1', exRef: state.txtExRef1, exKey: 'txtExRef1', sealEmpValue: state.txtSealByEmp1, breakEmpValue: state.txtBreakByEmp1, s1Value: state.txtForwarding1S1, s1Key: 'txtForwarding1S1', s2Value: state.txtForwarding1S2, s2Key: 'txtForwarding1S2', toggleEvent: ToggleFW1(), sealEvent: (n, id) => SealEmp1Selected(n, id), breakEvent: (n, id) => SealEmp1Selected(n, id, isBreak: true), width: width),
        const SizedBox(height: 10),
        _fwCard(context, bloc, state, fp, fwNum: 2, visible: state.visibleFW2, dropValue: state.dropdownValueFW2, dropKey: 'dropdownValueFW2', dateStr: state.dtpFW2date, checkValue: state.checkBoxValueFW2, checkKey: 'checkBoxValueFW2', dateKey: 'dtpFW2date', smkValue: state.txtSmk2, smkKey: 'txtSmk2', enRef: state.txtENRef2, enKey: 'txtENRef2', exRef: state.txtExRef2, exKey: 'txtExRef2', sealEmpValue: state.txtSealByEmp2, breakEmpValue: state.txtBreakByEmp2, s1Value: state.txtForwarding2S1, s1Key: 'txtForwarding2S1', s2Value: state.txtForwarding2S2, s2Key: 'txtForwarding2S2', toggleEvent: ToggleFW2(), sealEvent: (n, id) => SealEmp2Selected(n, id), breakEvent: (n, id) => SealEmp2Selected(n, id, isBreak: true), width: width),
        const SizedBox(height: 10),
        _fwCard(context, bloc, state, fp, fwNum: 3, visible: state.visibleFW3, dropValue: state.dropdownValueFW3, dropKey: 'dropdownValueFW3', dateStr: state.dtpFW3date, checkValue: state.checkBoxValueFW3, checkKey: 'checkBoxValueFW3', dateKey: 'dtpFW3date', smkValue: state.txtSmk3, smkKey: 'txtSmk3', enRef: state.txtENRef3, enKey: 'txtENRef3', exRef: state.txtExRef3, exKey: 'txtExRef3', sealEmpValue: state.txtSealByEmp3, breakEmpValue: state.txtBreakByEmp3, s1Value: state.txtForwarding3S1, s1Key: 'txtForwarding3S1', s2Value: state.txtForwarding3S2, s2Key: 'txtForwarding3S2', toggleEvent: ToggleFW3(), sealEvent: (n, id) => SealEmp3Selected(n, id), breakEvent: (n, id) => SealEmp3Selected(n, id, isBreak: true), width: width),
        const SizedBox(height: 10),
      ],
      if (state.visibleZB) ...[
        _zbCard(context, bloc, state, fp, 1), const SizedBox(height: 10), _zbCard(context, bloc, state, fp, 2), const SizedBox(height: 10),
      ],
      _sectionCard(children: [
        _sectionLabel('Boarding Officer 1'), _gap(),
        _searchField(
...
if (__navResult7 != null) { AppGlobals.SelectEmployeeList = __navResult7; }
        ...
        _editField(uniqueId: 'txt_amount1', hint: 'Amount 1', value: state.txtAmount1, enabled: !state.disabledAmount1, onChanged: (v) => bloc.add(UpdateTextField('txtAmount1', v))), _gap(),
        _editField(uniqueId: 'txt_portchargeref1', hint: 'Port Charges Ref', value: state.txtPortChargeRef1, enabled: fp["txtPortChargeRef1"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtPortChargeRef1', v))), _gap(),
        _divider(), _gap(),
        _sectionLabel('Boarding Officer 2'), _gap(),
        _searchField(
...
if (__navResult8 != null) { AppGlobals.SelectEmployeeList = __navResult8; }
        ...
        _editField(uniqueId: 'txt_amount2', hint: 'Amount 2', value: state.txtAmount2, enabled: !state.disabledAmount2, onChanged: (v) => bloc.add(UpdateTextField('txtAmount2', v))), _gap(),
        _editField(uniqueId: 'txt_portcharges', hint: 'Port Charges', value: state.txtPortCharges, enabled: fp["txtPortCharges"] == true, onChanged: (v) => bloc.add(UpdateTextField('txtPortCharges', v))),
      ]),
    ]);
  }
  ...
  Widget _fwCard(
      BuildContext context, SalesOrderAddBloc bloc, SalesOrderAddLoaded state, Map<String, bool> fp, {
        ...
        required String dateStr, required bool checkValue, required String checkKey, required String dateKey,
        required String smkValue, required String smkKey, required String enRef, required String enKey,
        required String exRef, required String exKey, required String sealEmpValue, required String breakEmpValue,
        required String s1Value, required String s1Key, required String s2Value, required String s2Key,
        required SalesOrderAddEvent toggleEvent, required SalesOrderAddEvent Function(String, int) sealEvent,
        required SalesOrderAddEvent Function(String, int) breakEvent, required double width,
      }) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: colour.border),
        boxShadow: const [BoxShadow(color: Color(0x0A1555F3), blurRadius: 8, offset: Offset(0, 2))],
      ),
      child: Column(children: [
        InkWell(
          onTap: () => bloc.add(toggleEvent), borderRadius: const BorderRadius.vertical(top: Radius.circular(16)),
          child: Container(
        ...
        if (visible)
          Padding(
            ...
            child: Column(children: [
              _dateCheckRow(context: context, label: "Date", dateStr: dateStr, checkValue: checkValue, checkKey: checkKey, dateKey: dateKey, enabled: fp[checkKey] == true, bloc: bloc), _gap(),
              _editField(uniqueId: 'txt_smk_$fwNum', hint: 'SMK NO $fwNum', value: smkValue, enabled: fp[smkKey] == true, onChanged: (v) => bloc.add(UpdateTextField(smkKey, v))), _gap(),
              Row(children: [
                Expanded(child: _editField(uniqueId: 'txt_enref_$fwNum', hint: 'R.No $fwNum', value: enRef, enabled: fp[enKey] == true, onChanged: (v) => bloc.add(UpdateTextField(enKey, v)))), const SizedBox(width: 10),
                Expanded(child: _editField(uniqueId: 'txt_exref_$fwNum', hint: 'EX.Ref $fwNum', value: exRef, enabled: fp[exKey] == true, onChanged: (v) => bloc.add(UpdateTextField(exKey, v)))),
              ]), _gap(),
              _searchField(
...
if (__navResult10 != null) { AppGlobals.SelectEmployeeList = __navResult10; }
              ...
              Row(children: [
                Expanded(child: _editField(uniqueId: 'txt_s1_$fwNum', hint: 'S1', value: s1Value, enabled: fp[s1Key] == true, onChanged: (v) => bloc.add(UpdateTextField(s1Key, v)))), const SizedBox(width: 10),
                Expanded(child: _editField(uniqueId: 'txt_s2_$fwNum', hint: 'S2', value: s2Value, enabled: fp[s2Key] == true, onChanged: (v) => bloc.add(UpdateTextField(s2Key, v)))),
              ]),
            ]),
          ),
      ]),
  ...
  Widget _zbCard(BuildContext context, SalesOrderAddBloc bloc, SalesOrderAddLoaded state, Map<String, bool> fp, int zbNum) {
    ...
    return _sectionCard(children: [
      ...
      _editField(uniqueId: 'txt_zbref_$zbNum', hint: 'ZB Ref $zbNum', value: refVal, enabled: fp[refKey] == true, onChanged: (v) => bloc.add(UpdateTextField(refKey, v))),
    ]);
  }
  ...
  void _showProductDialog(BuildContext context, SalesOrderAddLoaded state, int editIndex) {
    ...
    showDialog(barrierDismissible: false, context: context, builder: (ctx) => BlocProvider.value(value: bloc, child: _ProductDialog(activeField: _activeNumpadField)));
  }
  void _showPickUpList(BuildContext context, SalesOrderAddLoaded state) {
    final bloc = context.read<SalesOrderAddBloc>();
    showDialog(barrierDismissible: false, context: context, builder: (ctx) => BlocProvider.value(value: bloc, child: const _AddressListDialog(title: "PickUp Address & Qty List", isPickUp: true)));
  }
  void _showDeliveryList(BuildContext context, SalesOrderAddLoaded state) {
    final bloc = context.read<SalesOrderAddBloc>();
    showDialog(barrierDismissible: false, context: context, builder: (ctx) => BlocProvider.value(value: bloc, child: const _AddressListDialog(title: "Delivery Address & Qty List", isPickUp: false)));
  }
...
class _ProductDialogState extends State<_ProductDialog> {
  ...
  @override void initState() { super.initState(); _activeField = widget.activeField; }
  @override Widget build(BuildContext context) {
    final double width = MediaQuery.of(context).size.width;
    final double height = MediaQuery.of(context).size.height;
    return BlocBuilder<SalesOrderAddBloc, SalesOrderAddState>(builder: (context, state) {
      if (state is! SalesOrderAddLoaded) return const SizedBox();
      final bloc = context.read<SalesOrderAddBloc>();
      return Dialog(
        backgroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        child: Container(
          width: width * 0.95, height: height * 0.92, padding: const EdgeInsets.all(16),
          child: Column(children: [
            Row(children: [
            ...
            Expanded(flex: 3, child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              _searchableField(
                ...
                onSearch: () async {
                  final r = await Navigator.push(context, MaterialPageRoute(builder: (_) => const Product(Searchby: 1, SearchId: 0))); if (r != null) { AppGlobals.SelectProductList = r; }
                  if (r != null) {
            ...
            Expanded(flex: 4, child: Container(decoration: BoxDecoration(color: colour.surface, borderRadius: BorderRadius.circular(14), border: Border.all(color: colour.border)), padding: const EdgeInsets.all(8), child: Column(children: [
              _numRow(['1', '2', '3'], bloc), _numRow(['4', '5', '6'], bloc), _numRow(['7', '8', '9'], bloc), _numRow(['CLR', '0', 'C'], bloc),
              Row(children: [_numBtn('Add', () { bloc.add(AddProduct()); Navigator.of(context, rootNavigator: true).pop(); }, isAction: true), _numBtn('Reset', () => bloc.add(ClearProduct()), isAction: true)]),
            ]))),
          ]),
        ),
      );
            ...
            Expanded(
                child: addresses.isEmpty
                    ...
                    itemBuilder: (ctx, index) {
                      return InkWell(
                        onLongPress: () async {
                          final del = await ConfirmationMsgYesNo(context, "Are you sure to delete?");
            if (!context.mounted) return;
                        ...
                        child: Container(
                          ...
                          child: Row(children: [
                            ...
                            // 4. Delete Icon
                            GestureDetector(onTap: () async { final del = await ConfirmationMsgYesNo(context, "Are you sure to delete?");
