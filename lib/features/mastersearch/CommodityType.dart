...
class CommodityType extends StatefulWidget {
  final int Searchby;
  final int SearchId;

  const CommodityType({super.key, required this.Searchby, required this.SearchId});

  @override
  _CommodityTypestate createState() => _CommodityTypestate();
}

class _CommodityTypestate extends State<CommodityType> {
  bool progress = false;
  final txtSearch = TextEditingController();

  final List<String> CommodityList = <String>[
    "SHIP SPARE",
    "PROVISION",
    "GENERAL CARGO",
  ];
  ...
  void search(String value) {
    setState(() {
      if (value.trim().isEmpty) {
        ...
        filtersearchlist = CommodityList.where(
    ...
    if (filtersearchlist.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(
              txtSearch.text.isNotEmpty ? Icons.search_off : Icons.category_outlined,
              size: 64,
              color: Colors.grey.shade400,
            ),
            const SizedBox(height: 16),
            Text(
              txtSearch.text.isNotEmpty ? 'No match found' : 'No Commodity Types',
              style: GoogleFonts.lato(
                textStyle: TextStyle(
            ...
            child: Padding(
              ...
              child: ListTile(
                leading: CircleAvatar(
                  ...
                  child: const Icon(Icons.category, color: colour.commonColor), // Category icon
                ),
                title: Text(
                  item.toString(),
